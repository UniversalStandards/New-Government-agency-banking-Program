...
class Department(db.Model):
    ...
    def to_dict(self):
        ...
        return {
            ...
            'code': self.code,
            'description': self.description,
            'budget_allocated': float(self.budget_allocated) if self.budget_allocated else 0,
            'budget_spent': float(self.budget_spent) if self.budget_spent else 0,
            'budget_remaining': float(self.budget_remaining) if self.budget_remaining else 0,
            'is_active': self.is_active,
            'created_at': self.created_at.isoformat() if self.created_at else None
        }
