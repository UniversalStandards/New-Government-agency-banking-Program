...
class ModernTreasuryClient:
    ...
    def create_external_account(self, external_account_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def list_external_accounts(self, params: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        query_params = "&".join([f"{k}={v}" for k, v in (params or {}).items()])
        endpoint = f"/external_accounts?{query_params}" if query_params else "/external_accounts"
        return self._make_request("GET", endpoint)
