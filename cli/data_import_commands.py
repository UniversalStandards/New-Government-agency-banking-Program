...
def register_data_import_commands(app):
    ...
    @click.option(
        "--full", is_flag=True, help="Perform full sync instead of incremental"
    )
    @with_appcontext
    def sync(service, full):
        ...
        try:
            ...
            if service:
                click.echo(
                    f"Starting {'full' if full else 'incremental'} sync for {service}..."
                )
                result = sync_engine.sync_service(service, full_sync=full)
            else:
                click.echo(
                    f"Starting {'full' if full else 'incremental'} sync for all services..."
                )
            ...
            if isinstance(result, dict) and "success" in result:
                ...
                if result["success"]:
                    click.echo(
                        f"✅ Sync completed: {result['items_synced']} items synced"
                    )
                else:
                    click.echo(
        ...
        try:
            ...
            if "services" in status_data:
                ...
                for service in status_data["services"]:
                    ...
                    click.echo(
                        f"   Items Synced: {service.get('total_items_synced', 0)}"
                    )
                    click.echo(
                        f"   Success Rate: {service.get('success_rate', 0):.1f}%"
                    )
                    click.echo(
                        f"   Enabled: {'Yes' if service.get('sync_enabled') else 'No'}"
                    )

                    if service.get("last_error_message"):
                        click.echo(
                            f"   Last Error: {service['last_error_message'][:100]}..."
                        )
                    click.echo()

                click.echo(f"Total Services: {status_data.get('total_services', 0)}")
                click.echo(
                    f"Healthy Services: {status_data.get('healthy_services', 0)}"
                )
            else:
    ...
    def list_data(source, data_type, limit):
        ...
        try:
            ...
            data_items = sync_engine.get_imported_data(
                source=source, data_type=data_type, limit=limit
            )
