...
class ImportedData(Base):
    ...
    def to_dict(self):
        ...
        return {
            ...
            "external_created_at": (
                self.external_created_at.isoformat()
                if self.external_created_at
                else None
            ),
            "external_updated_at": (
...
class SyncStatus(Base):
    ...
    def success_rate(self):
        ...
        if self.total_sync_attempts == 0:
            return 0.0
        return (
            (self.total_sync_attempts - self.total_errors) / self.total_sync_attempts
        ) * 100
    ...
    def is_healthy(self):
        ...
        if self.last_successful_sync_at:
            ...
            age_minutes = (
                datetime.utcnow() - self.last_successful_sync_at
            ).total_seconds() / 60
