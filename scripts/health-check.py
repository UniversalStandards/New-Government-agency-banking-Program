...
def check_code_quality() -> Dict[str, Any]:
    ...
    checks['black_formatting'] = {
        ...
        'output': result['stdout'][:500] if result['stdout'] else 'Code is properly formatted'
    }
...
def check_performance() -> Dict[str, Any]:
    ...
    checks['codebase_metrics'] = {
        ...
        'average_file_size_kb': round(total_size / len(python_files) / 1024, 2) if python_files else 0
    }
...
def generate_health_report(results: Dict[str, Any]) -> str:
        ...
        if isinstance(checks, dict):
            for check_name, check_result in checks.items():
                ...
                if isinstance(check_result, dict):
                    if check_result.get('passed') or check_result.get('success') or check_result.get('connected'):
                        passed_checks += 1
    ...
    report_lines.append(f"## 🎯 Overall Health Score: {health_score}% ({passed_checks}/{total_checks} checks passed)")
    report_lines.append("")
    ...
    if health_score >= 90:
        ...
        report_lines.append("### 🚨 Status: CRITICAL - Multiple issues require immediate attention")
    ...
    for category, checks in results.items():
        ...
        if isinstance(checks, dict):
            for check_name, check_result in checks.items():
                status = "✅" if (isinstance(check_result, dict) and 
                                (check_result.get('passed') or check_result.get('success') or check_result.get('connected'))) or check_result is True else "❌"
