...
class BaseImporter(ABC):
    ...
    def __init__(self, config: ImportConfig, service_name: str):
        ...
        self.sync_stats = {"total_synced": 0, "errors": 0, "last_error": None}
    ...
    def sync(self, full_sync: bool = False) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Starting {'full' if full_sync else 'incremental'} sync for {self.service_name}"
        )
        ...
        try:
            ...
            duration = (datetime.now() - start_time).total_seconds()
            self.logger.info(
                f"Sync completed for {self.service_name}: {items_stored} items in {duration:.2f}s"
            )
    ...
    def validate_connection(self) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Connection validation failed for {self.service_name}: {e}"
            )
            return False
