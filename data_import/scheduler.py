...
class SyncScheduler:
    ...
    def start(self, check_interval_seconds: int = 60):
        ...
        self._thread = threading.Thread(
            target=self._run_scheduler, args=(check_interval_seconds,), daemon=True
        )
        self._thread.start()

        self.logger.info(
            f"Scheduler started with check interval of {check_interval_seconds} seconds"
        )
            ...
            try:
                ...
                if services_needing_sync:
                    ...
                    for service_name in services_needing_sync:
                        ...
                        try:
                            self.logger.info(
                                f"Starting scheduled sync for {service_name}"
                            )
                            result = self.sync_engine.sync_service(service_name)

                            if result["success"]:
                                self.logger.info(
                                    f"Scheduled sync completed for {service_name}: {result['items_synced']} items"
                                )
                                if self.on_sync_complete:
                                    self.on_sync_complete(service_name, result)
                            else:
                                self.logger.error(
                                    f"Scheduled sync failed for {service_name}: {result.get('error', 'Unknown error')}"
                                )
                            ...
                            self.logger.error(
                                f"Exception during scheduled sync for {service_name}: {e}"
                            )
                            if self.on_sync_error:
    ...
    def sync_now(
        self, service_name: Optional[str] = None, full_sync: bool = False
    ) -> dict:
