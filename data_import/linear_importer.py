...
class LinearImporter(BaseImporter):
    ...
    def _transform_project(self, project: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "teams": [
                team["name"] for team in project.get("teams", {}).get("nodes", [])
            ],
            "members": [
                member["name"] for member in project.get("members", {}).get("nodes", [])
            ],
    ...
    def _transform_team(self, team: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "members": [
                member["name"] for member in team.get("members", {}).get("nodes", [])
            ],
    ...
    def store_data(self, transformed_data: List[Dict[str, Any]]) -> int:
        ...
        for item in transformed_data:
            try:
                ...
                self.logger.debug(
                    f"Would store {item['data_type']}: {item.get('title', item.get('name', item['external_id']))}"
                )
                stored_count += 1
            except Exception as e:
                self.logger.error(
