...
class SyncEngine:
    ...
    def _initialize_importers(self):
        ...
        self.logger.info(
            f"Initialized {len(self.importers)} importers: {list(self.importers.keys())}"
        )
    ...
    def sync_service(
        self, service_name: str, full_sync: bool = False
    ) -> Dict[str, Any]:
        ...
        with Session(self.engine) as session:
            ...
            sync_status = (
                session.query(SyncStatus).filter_by(service=service_name).first()
            )
            if not sync_status:
            ...
            try:
                ...
                if result["success"]:
                    ...
                    if "transformed_data" in result:
                        self._store_imported_data(
                            session, service_name, result["transformed_data"]
                        )
                else:
                    # Update error statistics
                    sync_status.total_errors += 1
                    sync_status.last_error_at = datetime.utcnow()
                    sync_status.last_error_message = result.get(
                        "error", "Unknown error"
                    )
                ...
                return {"success": False, "error": str(e), "items_synced": 0}

    def _store_imported_data(
        self, session: Session, service_name: str, data_items: List[Dict[str, Any]]
    ):
        ...
        for item in data_items:
            try:
                ...
                existing_item = (
                    session.query(ImportedData)
                    .filter_by(source=service_name, external_id=item["external_id"])
                    .first()
                )
                ...
                self.logger.error(
                    f"Failed to store item {item.get('external_id', 'unknown')}: {e}"
                )
                continue
            ...
            if service_name:
                ...
                return {
                    ...
                    "healthy_services": sum(
                        1 for status in all_status if status.is_healthy
                    ),
                }
    ...
    def enable_service_sync(self, service_name: str, enabled: bool = True):
        ...
        with Session(self.engine) as session:
            ...
            if sync_status:
                ...
                session.commit()
                self.logger.info(
                    f"Sync {'enabled' if enabled else 'disabled'} for {service_name}"
                )
            else:
    ...
    def set_sync_interval(self, service_name: str, interval_minutes: int):
        ...
        with Session(self.engine) as session:
            ...
            if sync_status:
                ...
                session.commit()
                self.logger.info(
                    f"Sync interval set to {interval_minutes} minutes for {service_name}"
                )
            else:
                raise SyncError(f"Service {service_name} not found")
        ...
        with Session(self.engine) as session:
            for service_name in self.importers.keys():
                ...
                if sync_status.last_sync_at:
                    ...
                    if (
                        time_since_sync.total_seconds() / 60
    ...
    def validate_all_connections(self) -> Dict[str, bool]:
        ...
        for service_name, importer in self.importers.items():
            try:
                ...
                self.logger.error(
                    f"Connection validation failed for {service_name}: {e}"
                )
