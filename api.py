...
from models import db, User, Account, Transaction, Budget, BudgetItem, UserRole, AccountType, TransactionType, TransactionStatus
...
def get_dashboard_stats():
    ...
    try:
        ...
        monthly_income = sum(
            float(t.amount) for t in monthly_transactions 
            if t.transaction_type in [TransactionType.DEPOSIT, TransactionType.PAYMENT]
        )
        
        monthly_expenses = sum(
            float(t.amount) for t in monthly_transactions 
            if t.transaction_type in [TransactionType.WITHDRAWAL, TransactionType.PAYMENT]
        )
