    ...
    def generate_payroll_reports():
        ...
        with app.app_context():
            try:
                ...
                first_of_month = current_date.replace(
        ...
        with app.app_context():
            try:
                ...
                for budget in budgets:
                    ...
                    alerts.append(alert)
                    logger.warning(
                        f"Budget alert: {budget.name} at {utilization:.1f}% utilization"
                    )
    ...
    def generate_daily_financial_report():
        ...
        with app.app_context():
            try:
                ...
                logger.info(
                    f"Generated daily financial report: {report['total_transactions']} transactions, ${report['total_amount']:.2f}"
                )
                return report
...
def setup_automation_schedules():
    ...
    schedule.every().day.at("07:00").do(
        UtilityPaymentAutomation.process_overdue_notices
    )
    schedule.every().day.at("09:00").do(
        UtilityPaymentAutomation.auto_pay_recurring_utilities
    )

    # Reporting automation
    schedule.every().day.at("23:30").do(
        ReportingAutomation.generate_daily_financial_report
    )
