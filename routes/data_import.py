...
def trigger_sync():
    ...
    try:
        ...
        service = (
            request.json.get("service")
            if request.is_json
            else request.form.get("service")
        )
        full_sync = (
            request.json.get("full_sync", False)
            if request.is_json
            else request.form.get("full_sync") == "true"
        )
        ...
        if request.is_json:
            ...
            if result.get("success", False):
                flash(
                    f"Sync completed successfully: {result.get('message', '')}",
                    "success",
                )
            else:
...
def view_data():
    ...
    try:
        ...
        if not sched:
            return render_template(
                "data_import/data.html", error="Scheduler not available"
            )
        ...
        data_items = sched.sync_engine.get_imported_data(
            source=source, data_type=data_type, limit=per_page, offset=offset
        )

        # Get available sources and data types for filtering
        all_data = sched.sync_engine.get_imported_data(
            limit=1000
        )  # Get more for filtering options
        sources = list(set(item["source"] for item in all_data))
        data_types = list(set(item["data_type"] for item in all_data))

        return render_template(
            "data_import/data.html",
...
def api_data():
    ...
    try:
        ...
        # Get imported data
        data_items = sched.sync_engine.get_imported_data(
            source=source, data_type=data_type, limit=limit, offset=offset
        )
...
def config():
    ...
    try:
        ...
        if not sched:
            return render_template(
                "data_import/config.html", error="Scheduler not available"
            )
