...
def generate_fix():
    ...
    return jsonify(
        {"success": True, "fix": fix_data, "timestamp": datetime.utcnow().isoformat()}
    )
...
def get_agent_status():
    ...
    # Get recent tasks
    recent_tasks = Task.query.filter(Task.created_at >= seven_days_ago).all()

    # Count by status
    by_status = {"todo": 0, "in_progress": 0, "review": 0, "completed": 0, "blocked": 0}
    ...
    automation_stats = {
        ...
        "auto_fixes_applied": sum(
            1 for t in recent_tasks if "auto-fix" in (t.description or "").lower()
        ),
    ...
    return jsonify(
        {
            "status": "active",
            "period": "last_7_days",
            "task_stats": {"total_tasks": len(recent_tasks), "by_status": by_status},
